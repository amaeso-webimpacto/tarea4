...
class ModuloTarea4 extends Module
{
    ...
    public function hookActionValidateOrder($params)
    {
        ...
        $total_spent_by_customer = Db::getInstance()->getValue(
            sprintf(
                'SELECT SUM(total_paid) FROM ._DB_PREFIX_.orders WHERE id_customer = %d',
        ...
        $num_cupones = Db::getInstance()->getValue(
            sprintf(
                'SELECT COUNT (discountcode) FROM ._DB_PREFIX_.modulo_tarea4 WHERE id_customer = %d',
                (int)pSQL($customer_id)
            )
        );
        ...
        //Condiciçn para crear cupon y mandar un mail u otro
        if ($total_acumulate >= $minimo) {
            ...
            $insert = array(
                ...
                'discount_creation_date' => date('Y-m-d H:i:s'),
            ...
            Db::getInstance()->update('modulo_tarea4', $insert);
